{******************************************************************************}
{                                                                              }
{       Delphi Google Map Viewer Demo Script Demo                              }
{                                                                              }
{       Copyright (c) 2021-2024 (Ethea S.r.l.)  , maXbox5                      }
...
procedure FormDestroy(Sender: TObject);
begin
  //activeoleControl := nil;    to fix
  // EdgeGoogleMapViewer.Free;
   writeln('debug: googlemaps form free');
end;
...
begin
 ...
 EdgeGoogleMapViewer.Free;
   action:= caFree;
   EdgeGoogleMapViewer:= NIL;
   screen.cursor:= crDefault;
   //showmessage('free');
   writeln('GMaps Free and Closer test finished');
   //end else
     //Action:= caNone;
end;
...
//object formMain: TformMain
  ...
  if fileExists ('.\GAPIKEY.INC') then begin //{$I .\GAPIKEY.INC}
     ...
     EdgeGoogleMapViewer.RegisterGoogleMapsApiKey(GOOGLE_APIKEY);
...
with formmain do begin
 ...
 // EdgeGoogleMapViewer:= TEdgeGoogleMapViewer.create(formmain)
    ...
    RegisterGoogleMapsApiKey(GOOGLE_APIKEY);
    writ(apikey) 
    //Left:= 0;
    //setbounds
    //Top:= 209
    ...
    MapAddress:= 'Domkloster 4, 50667 Köln, Germany'
    ...
    //showmap2('Domkloster 4, 50667 Köln, Germany');
    mapzoom:= 6;
    //BeforeShowMap:= @EdgeGoogleMapViewerBeforeShowMap
    //showmap3(alat);
    GotoLocation(alat, true);
    Showmap3(alat);
    //BeforeShowMap:= @EdgeGoogleMapViewerBeforeShowMap
...
initialization
  ...
  writ(EdgeGoogleMapViewer.DefaultCustomMarkerJSON);
